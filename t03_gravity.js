...
function setup() {
	try {
		...
		rectangleSprite = new Sprite(windowWidth*0.5, windowHeight, 80, 120, 'd'); //'d' lets it move dynamically, ie. is affected by gravity AND sprites. use 'k' for just code.
		rectangleSprite.color = 'red';
		rectangleSprite.rotationSpeed = -1;
		rectangleSprite.vel.y = -3;
		

		cubeSprite = new Sprite(windowWidth*0.5, windowHeight, 0, 50, 50, 'n')
		cubeSprite.color = 'blue';
		cubeSprite.rotationSpeed = -5;
		

		circleSprite = new Sprite(windowWidth*0.5, windowHeight, 100, 80, 'k')
